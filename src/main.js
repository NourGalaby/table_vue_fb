    ...
    search: function () {
      ...
      this.$http.get('http://localhost:5000/search?token=' + document.getElementById('token').value + '&search_term=' + document.getElementById('search').value).then((dataSample) => {
        ...
        for (var colName in body) {
          if (body.hasOwnProperty(colName)) { //only pushes columns with special attributes 
            if (colName === 'picture') {
              ...
              columns.push({label: 'Page Name', field: colName, filterable: true , html: true})
            } else {
              columns.push({label: colName, field: colName, filterable: true})
            }
            var col = body[colName] 
            for (var id in col) { //for each id value in a column
              if (col.hasOwnProperty(id)) {
                var val = col[id] //
                if (!rows[id]) { //create new raw if it doesnt exist
                  rows[id] = {'id': id}
                }
                if (colName === 'picture') {
                  rows[id][colName] = '<img src ="' + val + '" />'
                } else if (colName === 'name') {
                  rows[id]['name'] = '<a href ="' + rows[id]['id'] + '" />' +  val
                } else {
                  rows[id][colName] = val
                }
              }
